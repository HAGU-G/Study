...
void Ring::Release()
{
	...
	{
		delete insideCircle;
		insideCircle = nullptr;
	}
	if (outsideCircle)
	{
		delete outsideCircle;
		outsideCircle = nullptr;
	}
}
...
{
	cout << "type: Ring" << std::endl << "position: " << ring.position << std::endl << "radius (in, out): (" << ring.insideCircle->GetRadius() << ", " << ring.outsideCircle->GetRadius() << ")";
	return cout;
}
