...
int main()
{
	...
	// │ ResourceMgr │
	// └─────────────┘
	auto& textureMgr = ResourceMgr<sf::Texture>::Instance();
	...
	*textureMgr.Get("graphics/background.png") >> spriteBackground;
	...
	*textureMgr.Get("graphics/bee.png") >> spBee;
	...
	sf::Sprite spTree;
	spTree.setTexture(*textureMgr.Get("graphics/tree2.png"));
	spTree.setOrigin(spTree.getTexture()->getSize().x / 2.f, 0);
	spTree.setPosition(vm.width / 2.f, 0);
	spBee.setScale(1, 1);

	for (int i = 0; i < NUM_OF_BRANCHES; ++i)
	{
		branches[i].setTexture(*textureMgr.Get("graphics/branch.png"));
	...
	sf::Sprite spPlayer;
	spPlayer.setTexture(*textureMgr.Get("graphics/player.png"));
	...
	sf::Sprite spAxe;
	spAxe.setTexture(*textureMgr.Get("graphics/axe.png"));
	...
	scoreMessage.setFont(*fontMgr.Get("fonts/KOMIKAP_.ttf"));
